...
This write up will focus on solving a single challenge of the qualifying event: Login Level 3.
...
Opening the Chrome DevTools quickly showed [a single JavaScript file](https://gist.github.com/birjj/15a1eb5fdafa112046804146042f18e9) containing the relevant logic, nice and unobscured, written in a `submit` handler on the login form. The rules our password has to fulfill can be summed up as:
...
We'd therefore need to limit the available possiblities as much as possible, in a way that could be implemented as early as possible in the brute force process. Although we could save a bit of work by considering the required character groups, it seemed like the most promising rule to cut down on the number of valid strings to test would be the base64 encoding. This was what I immediately jumped on.
...
3. Since the sum of all digits must be 27, an uneven number, the palindrome must be of uneven length (e.g. `abcba`, not `abccba`). Combined with knowing the length must be divisible by 3, and be between 10 and 20, this means that we know the password to be of length 15.
...
To use the above observations to cut down on our brute force work, we'll filter out any characters that encode to a non-lowercase base64 output. This means we'll only have to check strings that at the very least encodes to the right format. We can implement it using simple bit-checking, filtering out any characters that would create a 6-bit group that doesn't represent a lowercase letter in base64:
